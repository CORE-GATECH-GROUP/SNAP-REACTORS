...
for i in range(numGeoFactor):
    templates[i] =  S83D_Revised(fe, ce, ir, br, ugp, lgp, cds, config='C4',  xsLibrary="ENDF8", hasThermScatt=True, rotateDrumIndex = [6] ,  rotateDrumAngle = [105], drumGeomFactor = geoFactors[i])
    templates[i].setSettings(geoType='3D', nps = 1E+06, nact = 100, nskip=100, 
        xsAbsPath=acePath, plotOptions=([3, 1], 1000, [0, 0], 1), xsLibrary="ENDF8")
     
    sTest3D = Serpent(id="s8c4_drumSens{}".format(i+1))
    sTest3D.toSerpent(reactorState=coldpow, template=templates[i], baseFileName="s8c4_drumSens{}".format(i+1))
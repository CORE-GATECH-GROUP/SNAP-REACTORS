...
drumAngles = np.linspace(3, 120, 40)

templates = [0]*len(drumAngles)
serpObjs = [0]*len(drumAngles)

for i in range(len(drumAngles)):
    templates[i] =  S83D_Revised(fe, ce, ir, br, ugp, lgp, cds, config='C4',  xsLibrary="ENDF7.1", hasThermScatt=False, rotateDrumIndex = [1, 2, 3, 4, 5, 6] ,  rotateDrumAngle = [drumAngles[i],  drumAngles[i],  drumAngles[i],  drumAngles[i],  drumAngles[i],  drumAngles[i]])
    templates[i].setSettings(geoType='3D', nps = 1E+04, nact = 60, nskip=20, 
        xsAbsPath=acePath, plotOptions=([3, 3], 1000, [0, 0], 1, ['P', 'M']), xsLibrary="ENDF7.1")
    sTest3D = Serpent(id="s8c4_drumAni{}".format(i+1))
    sTest3D.toSerpent(reactorState=coldpow, template=templates[i], baseFileName="s8c4_drumAni{}".format(i+1))
